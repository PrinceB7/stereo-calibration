...
def read_camera_parameters(filename):
    ...
    intrinsic_matrix = np.array([[float(val) for val in content[1].split()[0:]],
                                    [float(val) for val in content[2].split()[0:]],
                                    [float(val) for val in content[3].split()[0:]]])
    distortion_coeffs = np.array([[float(val) for val in content[5].split()[0:]]])
...
if __name__ == '__main__':
    ...
    read_from_previous_camera_intrinsics = calibration_settings['frame_width']
    
    if not read_from_previous_camera_intrinsics:
        ...
        cmtx0, dist0 = read_camera_parameters("camera_parameters/camera0_intrinsics_0.dat")
        cmtx1, dist1 = read_camera_parameters("camera_parameters/camera1_intrinsics_0.dat")
